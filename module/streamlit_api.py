...
class StreamlitApp:
    ...
    def render_step2(self):
        ...
        self.render_memo_chat_interface(
            page_number="step2",
            memo_title="💭 思考メモ",
            memo_placeholder="AIとの対話を通じて考えたことや、目標についてのアイデアを自由にメモしてください...",
            memo_key="step2_memo",
            chat_history_key='dialogue_log',
            chat_input_key='goal_input',
            chat_placeholder='AIアシスタントからの質問に回答してください...',
    ...
    def render_step3(self):
        ...
        self.render_memo_chat_interface(
            page_number="step3",
            memo_title="💡 アイデア整理メモ",
            memo_placeholder="目標達成のための活動内容についてのアイデアや考えを自由にメモしてください...",
            memo_key="step3_memo",
            chat_history_key='dialogue_log_plan',
            chat_input_key='plan_input',
            chat_placeholder='あなたの回答を入力してください...',
            initial_message=initial_message
        )

        # 最終的な活動内容の入力エリア
        st.markdown("---")
        st.subheader("📋 最終的な活動計画")
    ...
    def save_memo_to_db(self, page: str, memo_key: str, memo_content: str):
        """メモをデータベースに保存"""
        try:
            # 既存のメモがあるかチェック
            existing_memo = self.conn.table("user_memos")\
                                   .select("id")\
                                   .eq("user_id", st.session_state.user_id)\
                                   .eq("page", page)\
                                   .eq("memo_key", memo_key)\
                                   .execute()
            
            if existing_memo.data:
                # 既存のメモを更新
                self.conn.table("user_memos")\
                        .update({"memo_content": memo_content})\
                        .eq("user_id", st.session_state.user_id)\
                        .eq("page", page)\
                        .eq("memo_key", memo_key)\
                        .execute()
                ...
                # 新規メモを作成
                self.conn.table("user_memos").insert({
                    "user_id": st.session_state.user_id,
                    "page": page,
                    "memo_key": memo_key,
            ...
            logging.error(f"メモ保存エラー: {e}", exc_info=True)
    
    def load_memo_from_db(self, page: str, memo_key: str):
        """データベースからメモを読み込み"""
        try:
            result = self.conn.table("user_memos")\
                           .select("memo_content")\
                           .eq("user_id", st.session_state.user_id)\
                           .eq("page", page)\
                           .eq("memo_key", memo_key)\
            ...
            if result.data:
                return result.data[0]["memo_content"]
            else:
                return ""
        except Exception as e:
            logging.error(f"メモ読み込みエラー: {e}", exc_info=True)
            return ""
    ...
    def render_memo_chat_interface(self,
                                  page_number: str,
                                  memo_title: str,
                                  memo_placeholder: str,
                                  memo_key: str,
        ...
        with col_chat:
            ...
            if submit_clicked and user_input.strip():
                ...
                # AIの応答を生成（メモの内容も考慮）
                context_with_memo = f"ユーザーのメモ内容: {st.session_state.get(memo_key, '')}\n\nユーザーの質問: {user_input}"
