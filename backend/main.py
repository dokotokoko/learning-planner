...
async def create_interest(
    ...
    try:
        ...
        result = supabase.table("interests").insert({
        ...
        if result.data and len(result.data) > 0:
            ...
            raise HTTPException(status_code=500, detail="興味関心の保存に失敗しました")
    except HTTPException:
        raise
    except Exception as e:
        handle_database_error(e, "興味関心の保存")

@app.get("/interests", response_model=List[InterestResponse])
async def get_interests(current_user: int = Depends(get_current_user)):
    """ユーザーの興味関心一覧取得（Supabase版）"""
    try:
        validate_supabase()
        
        result = supabase.table("interests").select("*").eq("user_id", current_user).order("created_at", desc=True).execute()
        ...
        handle_database_error(e, "興味関心の取得")
...
async def create_goal(
    ...
    try:
        ...
        # 興味関心IDを取得
        interest_result = supabase.table("interests").select("id").eq("user_id", current_user).eq("interest", goal_data.interest).execute()
        interest_id = interest_result.data[0]["id"] if interest_result.data else None
        ...
        if result.data and len(result.data) > 0:
            ...
            return GoalResponse(
                id=goal["id"],
                goal=goal["goal"],
                interest_id=goal["interest_id"] or 0,
                created_at=goal["created_at"]
            )
        else:
            raise HTTPException(status_code=500, detail="学習目標の保存に失敗しました")
    except HTTPException:
        raise
    except Exception as e:
        handle_database_error(e, "学習目標の保存")

@app.get("/goals", response_model=List[GoalResponse])
async def get_goals(current_user: int = Depends(get_current_user)):
    """ユーザーの学習目標一覧取得（Supabase版）"""
    try:
        validate_supabase()
        
        result = supabase.table("goals").select("*").eq("user_id", current_user).order("created_at", desc=True).execute()
        ...
        handle_database_error(e, "学習目標の取得")
...
async def create_learning_plan(
    ...
    try:
        ...
        # 目標IDを取得
        goal_result = supabase.table("goals").select("id").eq("user_id", current_user).eq("goal", plan_data.goal).execute()
        goal_id = goal_result.data[0]["id"] if goal_result.data else None
        
        result = supabase.table("learning_plans").insert({
            "user_id": current_user,
            "goal_id": goal_id,
            "nextStep": plan_data.nextStep
        }).execute()
        ...
        if result.data and len(result.data) > 0:
            ...
            return LearningPlanResponse(
                ...
                goal_id=plan["goal_id"] or 0,
                created_at=plan["created_at"]
            )
        else:
            raise HTTPException(status_code=500, detail="学習計画の保存に失敗しました")
    except HTTPException:
        raise
    except Exception as e:
        handle_database_error(e, "学習計画の保存")
...
async def get_learning_plans(current_user: int = Depends(get_current_user)):
    """ユーザーの学習計画一覧取得（Supabase版）"""
    try:
        validate_supabase()
        
        result = supabase.table("learning_plans").select("*").eq("user_id", current_user).order("created_at", desc=True).execute()
        ...
        handle_database_error(e, "学習計画の取得")
...
async def get_learning_path_reflection(current_user: int = Depends(get_current_user)):
    ...
    try:
        ...
        # Step 1: 興味関心
        interests_result = supabase.table("interests").select("interest").eq("user_id", current_user).order("created_at", desc=True).limit(1).execute()
        if interests_result.data:
            themes.append(StepTheme(
                step=1,
                theme="興味関心の特定",
                content=interests_result.data[0]["interest"]
            ))
        
        # Step 2: 学習目標
        goals_result = supabase.table("goals").select("goal").eq("user_id", current_user).order("created_at", desc=True).limit(1).execute()
        if goals_result.data:
            themes.append(StepTheme(
                step=2,
                theme="学習目標の設定",
                content=goals_result.data[0]["goal"]
            ))
        
        # Step 3: 学習計画
        plans_result = supabase.table("learning_plans").select("nextStep").eq("user_id", current_user).order("created_at", desc=True).limit(1).execute()
        if plans_result.data:
            themes.append(StepTheme(
                step=3,
                theme="学習計画の立案",
                content=plans_result.data[0]["nextStep"]
            ))
        
        # チャット履歴も含める（簡略版）
        chat_history = []
        chat_result = supabase.table("chat_logs").select("message").eq("user_id", current_user).order("created_at", desc=True).limit(10).execute()
        chat_history = [row["message"] for row in chat_result.data]
        ...
        handle_database_error(e, "学習の振り返り情報の取得")
